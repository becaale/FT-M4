  ...
  describe("Parte DOS", () => {
    describe("POST /ability", () => {
      ...
      it("should return status 201 and ability object if the ability was succesfully created", async () => {
        const res = await request(app).post("/ability").send({ name: "Fire Ball", mana_cost: 80.5 });
    ...
    describe("Multiple Routes", () => {
      ...
      it("should associate ability with character", async () => {
        const res = await request(app).put("/ability/setCharacter").send({ idAbility: 1, codeCharacter: "ONE" });
        expect(res.body).toEqual(
          expect.objectContaining({
            name: "Fire Ball",
            description: null,
